...
select concat_ws(' ',first_name, last_name) as Имя_Фамилия,
email as Почта, char_length(email) as Длина_Поля_email,
last_update::date as Последнее_Обновление
from customer
...
select left((split_part(email, '@', 1)),1) || right(lower(split_part(email, '@', 1)),-1),
left(upper(split_part(email, '@', 2)),1) || right((split_part(email, '@', 2)),-1)
from customer
